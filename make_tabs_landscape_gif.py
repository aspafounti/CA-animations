...
Matches the section1 standard: flat 1px grey outline (baked gradient border
stripped from source), section1-size cursor drawn at output resolution,
brake-in (ease-out) cursor glides. Transparent corners + animated cursor.
"""
from PIL import Image, ImageDraw, ImageChops
import os, math, re, subprocess
...
# landscape reframe: crop a 565x465 window centred on the content (tab bar +
# dropdown span svg y~172..471, centre ~321) so no interaction is clipped.
SVG_LAND_H = 465
CROP_Y0 = S(89)                              # 356
CROP_Y1 = CROP_Y0 + S(SVG_LAND_H)            # 356 + 1860 = 2216
OUT_W, OUT_H = 900, round(SVG_LAND_H * 900 / SVG_SQ)   # 900 x 741

# ── render + clean the 6 source frames (same method as section1: strip the
#    baked cursor + gradient border, we redraw both ourselves) ──────────────
WORK = "/tmp/tabs4b"; os.makedirs(WORK, exist_ok=True)
CURSOR = re.compile(r'<g filter="url\(#filter\d+_d_[^)]*\)">\s*<path d="[^"]*" fill="white"/>\s*<path d="[^"]*" fill="black"/>\s*</g>')
BORDER = re.compile(r'(<path\s+d="M15 0\.5H550[^"]*")\s+stroke="[^"]*"')
print("Rendering source frames…")
ST = {}
for n in range(1, 7):
    p = f"/Users/aspa.founti/Desktop/Tabs animation/03-Frame {n}.svg"
    txt = open(p).read(); txt = CURSOR.sub("", txt); txt = BORDER.sub(r'\1', txt)
    sp = f"{WORK}/S{n}.svg"; open(sp, "w").write(txt)
    subprocess.run(["qlmanage", "-t", "-s", "2260", "-o", WORK, sp], capture_output=True)
    ST[n] = Image.open(f"{WORK}/S{n}.svg.png").convert("RGB").crop((0, 0, SQ, SQ))
...
HOLD10   = 149    # HOLD: dropdown closed, Dashboard 10 selected, cursor stays put
SETTLE   = 162    # quick move: cursor rises to DB1 tab
HOVER1   = 166    # arrive + click, almost instantly
HOLD1    = 178    # HOLD: click landed (already reverted to DB1), cursor stays put (0.5s)
RESET    = 202    # move back to original rest position (1s)
TOTAL, FPS = 210, 24
def state(f):
    ...
    if f < GLIDE:                                                # glide down to DB10 (brake in, matches series)
        t = ease_out((f - HOLD_TOP) / (GLIDE - HOLD_TOP))
    ...
    if f < HOLD10:                                               # HOLD: dropdown closed, cursor stays at DB10
        return ST[5], DB10_D, False
    if f < SETTLE:                                               # quick move: cursor rises to DB1 tab
        ct = ease_out((f - HOLD10) / (SETTLE - HOLD10))
        tip = (lerp(DB10_D[0], DB1_TB[0], ct), lerp(DB10_D[1], DB1_TB[1], ct))
        return ST[5], tip, False
    if f < HOVER1:                                               # arrive + click, almost instantly (click switches immediately)
        return ST[6], DB1_TB, (f - SETTLE) < 3
    if f < HOLD1:                                                # HOLD: already reverted to DB1, cursor stays put (0.5s)
        return ST[1], DB1_TB, False
    if f < RESET:                                                # move back to original rest position (1s)
        t = ease_out((f - HOLD1) / (RESET - HOLD1))
        return ST[1], (lerp(DB1_TB[0], REST[0], t), lerp(DB1_TB[1], REST[1], t)), False
...
for f in range(0, TOTAL):
    ...
    im = img.copy()
    im = im.crop((0, CROP_Y0, SQ, CROP_Y1))          # landscape reframe
    out = im.resize((OUT_W, OUT_H), Image.LANCZOS)
    tx = S(tip[0]) * OUT_W / SQ
    ty = (S(tip[1]) - CROP_Y0) * OUT_H / (CROP_Y1 - CROP_Y0)
    draw_cursor(out, tx, ty, press)                  # cursor drawn at output scale, section1 size
    add_outline(out)
    frames.append(out)
...
montage.paste(frames[len(frames) * 45 // 100], (0, OUT_H))
...
out = "/Users/aspa.founti/Claude/section3.gif"
