"""
Landscape (565x465) tab-switch animation — My Dashboards <-> ExTrac Curated.
Mirrors make_tab_square_gif.py EXACTLY (dynamic tabs, hover-brighten, press-flash,
instant content swap, crisp thumbnails, EXTRC->EXTRAC fix, [8]/[21]/[12] counts).
Only the frame height and the card-title baseline differ from the square version.
Source SVGs already omit the intro paragraph + All/Home/Personal/Team filter pills.
Matches the section1 standard: flat 1px grey outline, section1-size cursor drawn
at output resolution, brake-in (ease-out) cursor glides.
"""
...
# ── render + clean the source frames (same method as section1) ──────────────
WORK = "/tmp/tabL2"; os.makedirs(WORK, exist_ok=True)
...
SRC = {'T1':'/Users/aspa.founti/Desktop/Tabs animation/Landscape gifs/01-Tab Animation.svg',
...
for k,p in SRC.items():
    ...
    RAW[k]=Image.open(f"{WORK}/{k}.svg.png").convert("RGB").crop((0,0,W,H))

F3, F4 = RAW['T1'], RAW['T2']                                    # My Dashboards / ExTrac Curated
for im in (F3, F4):
...
for _tx in (S(37), S(441)):                       # card 1 + peek card title left edges
    _d4.rectangle([_tx-S(3), S(299), _tx+S(212), S(318)], fill=(41,41,41))
    _d4.text((_tx, S(314)), "EXTRAC–Dashboard 01", font=_HBOLD, anchor="ls", fill=(204,204,204))

CONTENT = {"F3": F3, "F4": F4}

# ── tab styling (identical to the square gif) ────────────────────────────────
...
RAD=round(14.5*900/SVG_W)
def add_outline(im):
    ImageDraw.Draw(im).rounded_rectangle([0,0,899,im.size[1]-1],radius=RAD,outline=(74,74,74),width=1)

START=(S(480),S(115))
EXTRAC=tab_center(1); MYDASH=tab_center(0)

# ── timeline (identical structure to the square gif) ─────────────────────────
TOTAL=204; FPS=24
def get_state(f):
    content="F3"; md="selected"; ex="default"; cx,cy=START; pr=0.0
    if f<14: pass
    elif f<44:
        t=ease_out((f-14)/30); cx=lerp(START[0],EXTRAC[0],t); cy=lerp(START[1],EXTRAC[1],t)
        # default while moving — tab brightens only on arrival (hold phase below)
        ...
        t=ease_out((f-92)/30); cx=lerp(EXTRAC[0],MYDASH[0],t); cy=lerp(EXTRAC[1],MYDASH[1],t)
...
# ── render (thinned 12 fps, as the square) ───────────────────────────────────
print("Rendering…")
...
SENT=(255,0,255)
montage=Image.new("RGB",(OUT_W,OUT_H*2)); montage.paste(frames[0],(0,0)); montage.paste(frames[len(frames)//2],(0,OUT_H))
...
keep=Image.new("L",(OUT_W,OUT_H),0); ImageDraw.Draw(keep).rounded_rectangle([0,0,OUT_W-1,OUT_H-1],radius=RAD,fill=255)
...
out="/Users/aspa.founti/Claude/tab-landscape.gif"
